...
export class Sha256 {
  public static hash(msg: string): string {
    ...
    var M = new Array<Array<number>>(N);
    ...
    var W = new Array<number>(64); var a, b, c, d, e, f, g, h;
    for (var i = 0; i < N; i++) {
      ...
      for (var t = 0; t < 64; t++) {
        var T1: number = h + Sha256.Σ1(e) + Sha256.Ch(e, f, g) + K[t] + W[t];
  ...
  private static Ch(x: number, y: number, z: number) {
    return (x & y) ^ (~x & z);
  }

  private static Maj(x: number, y: number, z: number) {
    return (x & y) ^ (x & z) ^ (y & z);
  }

  private static toHexStr(n: number) {
  ...
  private static utf8Encode(msg: string): string {
